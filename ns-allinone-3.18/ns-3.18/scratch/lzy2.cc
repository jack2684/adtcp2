...
//			}
	...
	if (debug3)
		std::cout << "average queue size: " << q->m_qAvg << std::endl;
	...
	if (flowStartSign == 1) {
		if (stat_start[flowId] == 0)
			stat_start[flowId] = Simulator::Now().GetSeconds();
	...
	if (flowEndSign == 1) {
		if (stat_end[flowId] == 0)
			stat_end[flowId] = Simulator::Now().GetSeconds();

		std::cout << "Flow Finished: ID=" << flowId << " Duration="
				<< stat_start[flowId] << " to " << stat_end[flowId] << "\n";
...
//	   //
	...
	if (debug3)
		std::cout << "moment queue size: " << q->m_qs << " id= " << q->queueIp
				<< std::endl;
...
int main(int argc, char *argv[]) {
	...
	int totalFlowNum = 100;

	stat_start = (double *) malloc(1000 * sizeof(double));
	stat_end = (double *) malloc(1000 * sizeof(double));
	memset(stat_start, 0, 1000 * sizeof(double));
	memset(stat_end, 0, 1000 * sizeof(double));
	...
	char line[256] = { 0 };
	for (int i = 0; i < totalFlowNum - 1; i++) {
		int src, dst, sendtime;
		long flowsize;

		fp.getline(line, sizeof(line) - 1);
		sscanf(line, "%d %d %d %ld", &sendtime, &src, &dst, &flowsize);

		flowsize = (flowsize / MAX_PAYLOAD) + 1;
		flowsize /= 10;
		if (flowsize == 0)
			flowsize = 1;

		//              std::cout << "src = " << src << " " << "dst = " << dst << " "
		//                              << "flowsize = " << flowsize << " " << "flowid = " << id << " "
		//                              << "sendtime = " << sendtime << "\n";

		sendFlow(level3, src, dst, flowsize, id, (int) (sendtime / 10),
				GlobalEndTime, "10Mbps", sinkPort);
		id++;
	}
	...
	int level1_num = LEVEL1_NUM;
	int level2_num = LEVEL2_NUM;
	int level3_num = LEVEL3_NUM;

	index << level1_num << "," << level2_num << "," << level3_num
			<< totalFlowNum << "\n";

	for (int i = 1; i < totalFlowNum; i++) {

		if (stat_end[i] == 0)
			continue; //filter out missing flows;
//		myout << stat_end[i] << "," << stat_start[i] << ","
//				<< stat_end[i] - stat_start[i] << "\n";
