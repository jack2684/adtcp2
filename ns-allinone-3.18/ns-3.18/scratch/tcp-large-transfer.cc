...
int main (int argc, char *argv[])
{
  ...
  PacketSinkHelper sink ("ns3::TcpSocketFactory",
                         InetSocketAddress (Ipv4Address::GetAny (), servPort+1));
                ...
                uint16_t servPort)
{
  ...
  //std::cout << "Starting flow at time " <<  Simulator::Now ().GetSeconds () << std::endl;
...
{
	int host_id=localSocket->host_id;
    uint32_t currentTxBytes=currentTxBytesArray[host_id];
...
//  	std::cin.get();
	...
	myfile << "finish at: " << Simulator::Now ().GetSeconds ()
