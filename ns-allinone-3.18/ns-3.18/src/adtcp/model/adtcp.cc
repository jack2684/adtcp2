...
namespace ns3 {
  ...
  Fst::RmFlow(uint64_g flowId)
  {
    ...
    else
    {
      ...
      {        
        ...
        //std::list<Flow>::iterator fit = m_flowList.begin();
        //for(; fit != m_flowList.end(); fit++)
        //{std::cout << "$$$$$$$$$$$$$$$$$$$$$$$$$$ " << H2F::h2f[117967150]->m_qAvg << std::endl;
  ...
  Fst::ExcceedMaxTh()
  {
    if(debug2 && m_qAvg > m_maxTh) std::cout << "\t Fst::ExcceedMaxTh: " << m_qAvg << '\t' << m_maxTh << "\n";
  ...
  Fst::ExcceedMinTh()
  {
    if(debug2 && m_qAvg < m_minTh ) std::cout << "\t Fst::ExcceedMinTh: " << m_qAvg << '\t' << m_minTh << "\n";
